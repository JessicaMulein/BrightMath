...
import rgbM from "./rgb";
import xyzM from "./XYZ";
...
class GamutMapping {
  ...
  */
  ...
  LChMaxC(LCh, rgb) {
    ...
    let lab, xyz;
    if (rgb == null) { rgb = rgbM(); }
    LCh.C = 0;
    let step = 110;
    let validC = null;
    for (let n = 0; n <= 50; n++) {
      lab = LCh.Lab(lab);
      xyz = this.labCs.XYZ(lab, xyz);
      this.rgbCs.rgb(xyz, rgb);
      if (rgb.isValid()) {
        validC = LCh.C;
        LCh.C += step;
      } else {
    ...
    LCh.C = validC;
    if (validC != null) { // solution found
      return this.rgbCs.rgb((this.labCs.XYZ((LCh.Lab(lab)), xyz)), rgb);
    } else {
      return null;
    }
  }
...
module.exports = (rgbCs, labCs) => new GamutMapping(rgbCs, labCs);
